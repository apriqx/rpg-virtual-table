...
export default function TablePage() {
  ...
  async function handleTokenSubmit(fd) {
    const base = { name: fd.name, displayName: fd.displayName || null, bars: Array.isArray(fd.bars) ? fd.bars : [], statusMarkers: Array.isArray(fd.statusMarkers) ? fd.statusMarkers : [] };
    let payload;
    if (isMaster) {
      payload = { ...base, type: fd.type, imageUrl: fd.imageUrl || null, width: Number(fd.width), height: Number(fd.height), rotation: Number(fd.rotation) || 0, layer: Number(fd.layer), visible: fd.visible, locked: fd.locked, snapToGrid: fd.snapToGrid, lightRadius: Number(fd.lightRadius) || 0, visionRadius: Number(fd.visionRadius) || 0, ownerId: fd.ownerId || null, characterId: fd.characterId || null, showName: fd.showName !== false, opacity: Number(fd.opacity) || 1 };
    } else {
      payload = { ...base, imageUrl: fd.imageUrl || null };
    }
    try {
      if (editingToken) {
        const d = await api.tokens.update(tableId, activeMap.id, editingToken.id, payload);
        setTokens((p) => p.map((t) => (t.id === d.id ? d : t))); setSelectedToken(d);
      } else {
        const d = await api.tokens.create(tableId, activeMap.id, { ...payload, x: tokenClickPos?.x || 0, y: tokenClickPos?.y || 0 });
